 ...
 */
/proc/power_failure()
	priority_announce("Abnormal activity detected in [station_name()]'s powernet. As a precautionary measure, the station's power will be shut off for an indeterminate duration.", "Critical Power Failure", SSstation.announcer.get_rand_poweroff_sound()) //REVSTATION EDIT: SSstation.announcer.get_rand_ion_sound() - original: ANNOUNCER_POWEROFF
 ...
 */
/proc/power_restore()
	priority_announce("Power has been restored to [station_name()]. We apologize for the inconvenience.", "Power Systems Nominal", SSstation.announcer.get_rand_poweron_sound()) //REVSTATION EDIT: SSstation.announcer.get_rand_poweron_sound() - original: ANNOUNCER_POWERON
	for(var/obj/machinery/power/apc/C as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/apc))
		if(C.cell && is_station_level(C.z))
...
/proc/power_restore_quick()
	priority_announce("All SMESs on [station_name()] have been recharged. We apologize for the inconvenience.", "Power Systems Nominal", SSstation.announcer.get_rand_poweron_sound()) //REVSTATION EDIT: SSstation.announcer.get_rand_poweron_sound() - original: ANNOUNCER_POWERON
	var/list/all_smes = SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/power/smes)
	for(var/obj/machinery/power/smes/smes as anything in all_smes)
		if(!is_station_level(smes.z))
			continue
